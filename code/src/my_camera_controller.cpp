...
namespace cgp
{
	...
	void my_camera_controller::set_rotation_axis(vec3 const& rotation_axis)
	{
		camera_model.set_rotation_axis(rotation_axis);
	}
	void my_camera_controller::set_rotation_axis_x()
	{
		camera_model.set_rotation_axis({ 1,0,0 });
	}
	void my_camera_controller::set_rotation_axis_y()
	{
		camera_model.set_rotation_axis({ 0,1,0 });
	}
	void my_camera_controller::set_rotation_axis_z()
	{
		camera_model.set_rotation_axis({ 0,0,1 });
	}

	void my_camera_controller::look_at(vec3 const& eye, vec3 const& center, vec3 const& )
	{
	...
	{
		std::string doc;
		doc += "Custom camera controller - Camera that rotates around its own position using Euler angle (XYZ/rool-pitch-yaw Trait-Bryan convention).\n";
		doc += "Control: \n";
